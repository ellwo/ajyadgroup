@extends('backend.layouts.backend-layout', ['class' => 'g-sidenav-show bg-gray-100 rtl', 'title' => 'ادارة الجوازات'])
@section('content')
    <!-- End Navbar -->
                ...
                <div class="card">
                    <div dir="rtl" class="pb-0 card-header">
                        ...
                        <div class=" container p-4 m-4 text-center bg-white rounded ">
                            ...
                            <div id="upload">
                                <label for="fileInput" class="form-label">يرجى اختيار ملف الاكسل بالتنسيق المعروض في الجدول
                                    ادناه </label>
            ...
            if (file.type == "application/vnd.ms-excel" || file.type ==
                ...
                reader.onload = function(e) {
                    ...
                    if (data.length < 0) {
                        ...
                        alert("ليس هناك اي سجلات !");
                        $("#fileInput").val('');

                    } else if (data[0].length != 8) {
                        alert(" يجب ان يحتوي ملف الاكسل على 8 اعمدة فقط");

                        window.removeEventListener('beforeunload', toggleReload);
                        $("#fileInput").val('');
            ...
            await $.ajax({
                ...
                success: function(data) {
                    ...
                    elem.innerHTML = width.toFixed(2) +
                        "%";
                    elemtext.innerHTML = (uploaded_counter) + "/" + all_data.length + "  " + width.toFixed(
                            2) +
                        "%";
