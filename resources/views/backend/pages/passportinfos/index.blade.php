...
<x-auth-session-status class="mb-4" class="alert-primary" :title="session('title')??'ملاحظة !'" :status="session('status')" />
    ...
    <div class=" container p-4 m-4 text-center bg-white rounded ">
        ...
        <div id="upload">
            <label for="fileInput" class="form-label">يرجى اختيار ملف الاكسل</label>
    ...
    fileInput.addEventListener('change', function(e) {
        ...
        if (file.type == "application/vnd.ms-excel" || file.type ==
            ...
            reader.onload = function(e) {
                ...
                var btnhtml =
                    '<button id="sendbtn" onClick="uploadeData()" type="button" class="btn btn-primary mt-4 pr-4 pl-4">ارسال</button>';
        ...
        await $.ajax({
            ...
            success: function(data) {
                ...
                if (uploaded_counter == all_data.length) {
                    width = 100;
                    $("#upload").append(
                        '<div class="alert alert-success alert-dismissible fade show ss"><button type="button" class="btn-close" data-bs-dismiss="alert">&times;</button><strong>Success!</strong> تم ارسال وحفظ  ' +
                ...
                elem.innerHTML = (uploaded_counter) + "/" + all_data.length + "  " + width.toFixed(2) +
                    "%";
